...
if (isset($_POST["submit_post"])) {
    ...
    if ($Posts->GetPostInfo($thread_id)["locked"]) {
        Redirect("error?error=THREAD_LOCKED");
    }
    if ($Posts->ReplyToThread($thread_id, $poster_name, $email, $subject_name, $message, $board, $files, $password, $salted_ip[0], $country_iso) == "BAN") {
        $ban_info = $Posts->CheckIfUserIsBanned($salted_ip[0], $board);
        if ($ban_info) {
            Redirect("error?error=BANNED&reason=" . $ban_info[0] . "&is_global=" . $ban_info[1] . "&board=" . $board);
        }
